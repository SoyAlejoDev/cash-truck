...
// Reducer function
const appReducer = (state: AppState & { loading: boolean }, action: Action): AppState & { loading: boolean } => {
  switch (action.type) {
    case 'INITIALIZE':
    ...
    case 'DELETE_EXPENSE': {
      ...
      return {
        ...
        weeks: weeks.map(week => ({
          ...week,
          expenses: week.expenses.filter(expense => expense.id !== action.payload)
        }))
      };
    }
    ...
    case 'DELETE_INCOME': {
      ...
      return {
        ...
        weeks: weeks.map(week => ({
          ...week,
          incomes: week.incomes.filter(income => income.id !== action.payload)
        }))
      };
    }
    ...
    case 'SELECT_WEEK': {
      return {
        ...state,
        weeks: state.weeks.some(w => w.id === action.payload.id)
          ? state.weeks
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    if (user) {
      ...
      dispatch({ type: 'INITIALIZE', payload: { weeks: [], currentWeekId: null } });
    }
  }, [user]);
